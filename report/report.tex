...
The actor model is composed by fully-connected layers followed by Leaky Rectified
Linear Unit activation function and then by batch normalization, because it
gives better training speed and efficiency. Why Leaky ? Because the ReLU
function is a function which makes weights equal to 0 when they are negative.
The problem with that is that a lot of information can be forgotten and we then
face a problem of dying neurons. The Leaky ReLU is less severe and defined as
follow: $f(x) = max(x, \alpha*x)$ where $\alpha <= 1$. 
...
action. The state input is followed by several dense layers activated by Leaky ReLU
and batch normalization. The action input is concat just before the last layer
of the state network. That layer is also followed by Leaky ReLU activation and
...
\begin{table}[ht]
  ...
  \begin{adjustbox}{width=1\textwidth}
    \begin{tabular}{ |c|c|c| }
      ...
      pass\_per\_epoch & Number of action during one epoch & 200 \\ 
      noise\_threshold & At which epochs the program stops adding noise to actions & 75 \\
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=.5\textwidth]{critic_graph}
  \caption{Critic network graph}
  \label{fig:criticgraph}
\end{figure}

The fig. \ref{fig:criticgraph} shows the model of the critic network. This is
the model I use for validating my results but I can easily add from hyper
parameters new layers, batch normalization or dropout operations, but the action
will always be added just before the last layer.

\begin{figure}[ht]
  \centering
  \includegraphics[width=.5\textwidth]{actor_graph}
  \caption{Actor network graph}
  \label{fig:actorgraph}
\end{figure}

The fig. \ref{fig:actorgraph} shows the model of the actor network. As critic
networks, the model is easily customizable and looking a lot like crtic model.
...
performance. However, it is interesting to see how well it will performed
without it. For dropout layer, I don't think forgetting some informations is a
good trick to tackle that kind of problem because usually, this technique is
used to prevent overfitting. Since DDPG is a deterministic policy, I don't think
overfitting will be a crucial issue.

\begin{figure}[ht]
  \begin{subfigure}[t]{.5\textwidth}
    \centering
    \includegraphics[width=1.\textwidth]{bn_avgreward}
    \caption{}
    \label{fig:bn_avgreward}
  \end{subfigure}%
  ~
  \begin{subfigure}[t]{.5\textwidth}
    \centering
    \includegraphics[width=1.\textwidth]{bn_loss}
    \caption{}
    \label{fig:bn_loss}
  \end{subfigure}
  \caption{Comparison of learning with and without batch normalization}
  \label{fig:bn_tweakinggraph}
\end{figure}

\begin{figure}[ht]
  \begin{subfigure}[t]{.5\textwidth}
    \centering
    \includegraphics[width=1.\textwidth]{dropout_avgreward}
    \caption{}
    \label{fig:bn_avgreward}
  \end{subfigure}%
  ~
  \begin{subfigure}[t]{.5\textwidth}
    \centering
    \includegraphics[width=1.\textwidth]{dropout_loss}
    \caption{}
    \label{fig:bn_loss}
  \end{subfigure}
  \caption{Comparison of learning with and without dropout}
  \label{fig:bn_tweakinggraph}
\end{figure}
...
architecture and their parameters. My agent started to started to learn how to
...
actually using it). In contrary, the most chanllenging part was to actually validate my network. The
difficulty came from the fact that it was hard to determine if a training was bad
because my network was not fitted the problem or because I didn't let it train enough.
