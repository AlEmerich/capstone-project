...
\begin{figure}[ht]
  \includegraphics[width=1.\textwidth]{noise_cancellation}
...
diverges very fast. I also add L2 weight deway to the critic network to
...
and then the loss diverges and the reward never increases, showing that the
noise process is either not noisy enough, or not enough suited to the environment.
Since I just want to validate the algorithm and not the model, the fact that
loss of the critic network is decreasing and the average reward is increasing
the first 1000 epochs shows that my model is able to learn.
...
networks, the model is easily customizable and looking a lot like critic model.
...
could help to not ave Q output in a local maxima.
