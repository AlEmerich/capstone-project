...
the actor network. Here the complete code:
...
environment. It was a decaying asymptotic-to-0 function used to generate less
...
\begin{itemize}
\item \textbf{reset}: \verb?true? to reset the environment at final states,
  \verb?false? otherwise.
\item \textbf{render}: \verb?true? to render the environment, \verb?false?
  otherwise. Roboschool has a bug which lead to give vector of \verb?inf? after
  the second reset when render, so when I train the networkds, I don't render
  it.
\item \textbf{plot}: \verb?tensorflow? to visualize plot with tensorboard,
  \verb?matplotlib? to render plot with matplotlib.
\item \textbf{train}: \verb?true? to train network, \verb?false? otherwise.
\item \textbf{noisy}: \verb?true? to add noise to action output, \verb?false?
  otherwise.
\item \textbf{load\_weights}: \verb?true? to load previously saved weight from
  previous training session, \verb?false? otherwise.
\item \textbf{batch\_size}: \verb?integer?, number of samples to get from memory.
\item \textbf{epochs}: \verb?integer?, number of epochs to act.
\item \textbf{noise\_threshold}; \verb?integer?, when epochs is hight than this
  number, the program stops adding noise to the action output.
\item \textbf{actor\_learning\_rate}: \verb?float?, learning rate for the actor
  network.
\item \textbf{critic\_learning\_rate}: \verb?float?, learning rate for the critic
  network.
\item \textbf{actor\_batch\_norm}: \verb?true? to add batch normamization after
  dense layers of the actor network, \verb?false? otherwise.
\item \textbf{critic\_batch\_norm}: \verb?true? to add batch normamization after
  dense layers of the critic network, \verb?false? otherwise.
\item \textbf{tau}: \verb?float?, used in the soft target update function.
\item \textbf{gamma}: \verb?float?, discounted factor.
\item \textbf{actor\_layers}: \verb?list of integer?, define the structure of the
  actor network, the depth and the number of unit in each layer.
\item \textbf{critic\_layers}: \verb?list of integer?, define the structure of the
  critic network, the depth and the number of unit in each layer.
\item \textbf{action\_range}: \verb?integer?, size of the action range to normalize
  it between 0 and 1.
\item \textbf{state\_range}: \verb?integer?, size of the state range to normalize
  it between 0 and 1.
\item \textbf{reward\_multiply}: \verb?integer?, multiply every reward with to
  reward more when good action is taken and less when a bad one is taken.
\item \textbf{dropout}: \verb?float?, percent of dropout to use after dense
  layers. 0 to not add dropout.
\end{itemize}
